...
$previewrows = optional_param('previewrows', 200, PARAM_INT);

@set_time_limit(60*60); // 1 hour should be enough
raise_memory_limit(MEMORY_HUGE);

require_login();
admin_externalpage_setup('regusers');
...
if (empty($iid)) {
    ...
    if (!empty($_POST['raw_csv_content'])) {
        $iid = csv_import_reader::get_new_iid('uploaduser');
        $cir = new csv_import_reader($iid, 'uploaduser');

        $content = $_POST['raw_csv_content'];
        ...
        echo $OUTPUT->heading(get_string('regusers', 'tool_regusers'));
